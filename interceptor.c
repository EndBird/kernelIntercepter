...
MODULE_AUTHOR("Brian Huang and Minh Huynh");
MODULE_LICENSE("GPL");
...
asmlinkage long interceptor(struct pt_regs reg) {
	long returnLong;
	//sysc is on ax according to .h file
	int sysc = (int) reg.ax;
	//Add locks
	spin_lock(&calltable_lock);
	spin_lock(&pidlist_lock);
	//check if its being monitored
	if (check_pid_monitored(sysc, current->pid)) {
		//Log the message using func from header
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si,
				reg.di, reg.bp);
	}
	//Call the actual system call
	returnLong = table[sysc].f(reg);
	//Remove locks
	spin_unlock(&pidlist_lock);
	spin_unlock(&calltable_lock);
	//return the system call stuff
	return returnLong;
}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	//Intercepting/Deintercepting
	if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) {
		//If not root, ask for permissions
		if (current_uid() != 0) {
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EPERM;
		...
		if (cmd == REQUEST_SYSCALL_INTERCEPT) {
			...
			table[syscall].f = sys_call_table[syscall]; //save original function.
			sys_call_table[syscall] = &interceptor; //pointer to new function.
			table[syscall].intercepted = 1; //now it is intercepted.
			set_addr_ro((unsigned long) sys_call_table);

		} else { //deintercept
			if (table[syscall].intercepted == 0) { //cannot deintercept something that has not been intercepted.
				spin_unlock(&pidlist_lock);
				spin_unlock(&calltable_lock);
				return -EINVAL;
			}
	...
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {
		if (table[syscall].intercepted == 0) { //cannot stop/start monitor pid for something that has not been intercepted.
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EINVAL;
		} else if (pid != 0
				&& (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL || pid < 0)) {
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EINVAL; //check if pid is valid
		} else if (pid == 0 && current_uid() != 0) {
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EPERM; //since it's valid, is the pid 0? if 0, then uid must be 0.
		}

		else if (current_uid() != 0 && pid != 0
				&& check_pid_from_list(pid, current->pid) != 0) { //put locks here.
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EPERM; //else if, check if two pids have same owners.
		...
		if (cmd == REQUEST_START_MONITORING) {
			if (check_pid_monitored(syscall, pid) == 1) {
				spin_unlock(&pidlist_lock);
				spin_unlock(&calltable_lock);
				return -EBUSY; //so pid is monitored by syscall
			}
			if (current_uid() == 0 && pid == 0) {
				destroy_list(syscall); //to empty the list since we don't need it.
				table[syscall].monitored = 2;

			} else {
				//Add it to list
				if (add_pid_sysc(pid, syscall) != 0) {
					//Fail means out of mem
					spin_unlock(&pidlist_lock);
					spin_unlock(&calltable_lock);
					return -ENOMEM;
				}
			...
			if (pid == 0) {
				...
				//If not intercepted or not monitored
				if (table[syscall].intercepted == 0
						|| check_pid_monitored(syscall, pid) == 0) {
					spin_unlock(&pidlist_lock);
					spin_unlock(&calltable_lock);
					return -EINVAL; //so pid is not monitored by syscall
				}
				//unlock
				spin_unlock(&pidlist_lock);
				spin_unlock(&calltable_lock);
...
static int init_function(void) {
	int i = 0;
	//Save custom call and exit group
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group];
	//Lock
	spin_lock(&calltable_lock);
	spin_lock(&pidlist_lock);
	//Init mytable in table
	set_addr_rw((unsigned long) sys_call_table);
	//Replace the calls with my calls
...
static void exit_function(void) {
	...
	set_addr_rw((unsigned long) sys_call_table);
	//Return the system call and exit group we hijacked
