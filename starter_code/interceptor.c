...
/* Symbol that allows access to the kernel system call table */ //symbol = variable//in each index is a function pinter that represents the handler(is the function) for spec syscall.
...
/*To intervcept the system call is to to replace the handler with your own.
 ...
 * requested to be monitored is owned by the requesting process. Minh: is owner also the same as moniterer?
...
/**  replace exitgroup completely.  do my ownt thing first and then call the real exit group
